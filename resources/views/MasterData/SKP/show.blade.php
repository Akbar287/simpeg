                ...
                <form action="{{ url('/skp') }}" method="post">@csrf
                    <div class="row">
                        ...
                        <div class="col-12">
                            ...
                            <a href="{{ url('skp/' .  $employeeWorkObjective->employee_work_objective_id . '/edit') }}" title="Ubah Data" class="btn btn-success">Ubah</a>
                            ...
                            <button type="button" title="Hapus" class="btn btn-danger" onclick="(confirm('Sasaran Kinerja Pegawai akan dihapus!\nLanjutkan?')) ? document.getElementById('deleteSKP').submit() : ''">Hapus</button>
                        </div>
                    </div>
                </form><form action="{{ url('skp/' . $employeeWorkObjective->employee_work_objective_id) }}" method="post" id="deleteSKP">@csrf @method('delete')</form>
            </div>
        </div>
    </div>
</div>
